...
from core.csv_tools import export_clients_csv, import_clients_csv_standard
...
class ClientsView:
    def __init__(self, page: ft.Page, session_user, notify: Callable[[str], None]) -> None:
        ...
        self.current_query = ''
        self.current_page = 1
        self.page_size = self.settings.get_int_value('clients_page_size', default=10, min_value=5, max_value=50)
        self.current_region = self.settings.get_value('clients_region_filter', ALL_REGIONS_KEY)
        self.current_province = self.settings.get_value('clients_province_filter', ALL_PROVINCES_KEY)
        saved_sort = self.settings.get_value('clients_sort', 'name_asc')
                    ...
                    controls=[
                        ...
                        ft.Container(
                            ...
                            content=ft.Column(
                                ...
                                controls=[
                                    ft.IconButton(icon=ft.Icons.ADD, icon_color=ft.Colors.WHITE, tooltip='Nuovo cliente', on_click=lambda _: self.open_dialog()),
                                    ft.IconButton(icon=ft.Icons.UPLOAD_FILE, icon_color=ft.Colors.WHITE, tooltip='Import CSV clienti', on_click=self._open_import_csv_dialog),
                                    ft.IconButton(icon=ft.Icons.DOWNLOAD, icon_color=ft.Colors.WHITE, tooltip='Export CSV clienti', on_click=self._open_export_csv_dialog),
                                ],
                            ),
                        ),
                    ],
    ...
    def _on_region_change(self, e: ft.ControlEvent) -> None:
        ...
        self.settings.set_value('clients_region_filter', self.current_region)
        self.current_province = ALL_PROVINCES_KEY
        self.settings.set_value('clients_province_filter', self.current_province)
        self._load_province_options()
        self.current_page = 1
        self._reload()
        self.page.update()
    ...
    def _on_sort_change(self, e: ft.ControlEvent) -> None:
        ...
        self.settings.set_value('clients_sort', self.current_sort)
        self.current_page = 1
        self._reload()
        self.page.update()
    ...
    def _on_page_size_change(self, e: ft.ControlEvent) -> None:
        ...
        self.settings.set_value('clients_page_size', str(self.page_size))
        self.current_page = 1
        self._reload()
        self.page.update()
    ...
    def _reload(self) -> None:
        ...
        rows, total = self.repo.list_clients(
                    ...
                    ft.DataCell(
                        ft.Row(
                            ...
                            controls=[
                                ...
                                ft.IconButton(icon=ft.Icons.SWAP_HORIZ, icon_color=ft.Colors.WHITE, tooltip='Sposta in Contatti', on_click=lambda _, cid=r['id']: self._confirm_move_to_contacts(cid)),
    ...
    def _confirm_delete(self, client_id: int) -> None:
        def do_delete(_):
            self.repo.delete_client(client_id, self.user.id)
            self._close()
            self._reload()
            self.page.update()
            self.notify('Cliente eliminato')
        ...
        self._dialog = ft.AlertDialog(bgcolor=ui_theme.CARD_BG, barrier_color=ft.Colors.with_opacity(0.55, ft.Colors.BLACK), title_text_style=ft.TextStyle(color=ft.Colors.WHITE, weight=ft.FontWeight.W_600), content_text_style=ft.TextStyle(color=ft.Colors.WHITE), 
            modal=True,
            title=ft.Text('Elimina cliente'),
            content=ft.Text('Confermi eliminazione cliente? I siti resteranno senza cliente assegnato.'),
    ...
    def _confirm_move_to_contacts(self, client_id: int) -> None:
        def do_move(_):
            try:
                self.repo.move_to_contacts(client_id, self.user.id)
            ...
            self._reload()
            self.page.update()
            self.notify('Cliente spostato in Contatti')

        self._dialog = ft.AlertDialog(bgcolor=ui_theme.CARD_BG, barrier_color=ft.Colors.with_opacity(0.55, ft.Colors.BLACK), title_text_style=ft.TextStyle(color=ft.Colors.WHITE, weight=ft.FontWeight.W_600), content_text_style=ft.TextStyle(color=ft.Colors.WHITE), 
            modal=True,
            title=ft.Text('Sposta in Contatti'),
            content=ft.Text('Confermi lo spostamento del cliente nella sezione Contatti?'),
    ...
    def _open_export_csv_dialog(self, _):
        path_field = _style_text_field(ft.TextField(label='Percorso file CSV export', value='exports/clients_export.csv', width=560))
        ...
        def do_export(_):
            ...
            if not str(target):
                ...
                return
            count = export_clients_csv(target)
            self._close()
            self.notify(f'Export clienti completato: {count} record in {target}')

        self._dialog = ft.AlertDialog(bgcolor=ui_theme.CARD_BG, barrier_color=ft.Colors.with_opacity(0.55, ft.Colors.BLACK), title_text_style=ft.TextStyle(color=ft.Colors.WHITE, weight=ft.FontWeight.W_600), content_text_style=ft.TextStyle(color=ft.Colors.WHITE), 
            modal=True,
            title=ft.Text('Export CSV clienti'),
    ...
    def _open_import_csv_dialog(self, _):
        path_field = _style_text_field(ft.TextField(label='Percorso file CSV import', value='exports/clients_export.csv', width=560))
        ...
        def do_import(_):
            ...
            if not source.exists():
                ...
                return
            count = import_clients_csv_standard(source, self.user.id)
            self._close()
            self._load_region_options()
            self._load_province_options()
            self._reload()
            self.page.update()
            self.notify(f'Import clienti completato: {count} record')

        self._dialog = ft.AlertDialog(bgcolor=ui_theme.CARD_BG, barrier_color=ft.Colors.with_opacity(0.55, ft.Colors.BLACK), title_text_style=ft.TextStyle(color=ft.Colors.WHITE, weight=ft.FontWeight.W_600), content_text_style=ft.TextStyle(color=ft.Colors.WHITE), 
            modal=True,
            title=ft.Text('Import CSV clienti'),
            content=ft.Container(width=700, content=ft.Column(tight=True, controls=[path_field])),
            actions=[ft.TextButton('Chiudi', on_click=lambda _: self._close()), ft.ElevatedButton('Importa', on_click=do_import)],
        )
        self.page.show_dialog(self._dialog)

    def open_dialog(self, client_id: int | None = None, read_only: bool = False) -> None:
        initial = self.repo.get_client(client_id) if client_id else {}
        ...
        def save(_):
            ...
            if client_id:
                self.repo.update_client(client_id, payload, self.user.id)
                self.notify('Cliente aggiornato')
            else:
                self.repo.create_client(payload, self.user.id)
                self.notify('Cliente creato')
            self._close()
            self._reload()
            self.page.update()
        ...
        self._dialog = ft.AlertDialog(bgcolor=ui_theme.CARD_BG, barrier_color=ft.Colors.with_opacity(0.55, ft.Colors.BLACK), title_text_style=ft.TextStyle(color=ft.Colors.WHITE, weight=ft.FontWeight.W_600), content_text_style=ft.TextStyle(color=ft.Colors.WHITE), 
            modal=True,
            title=ft.Text('Cliente' if client_id else 'Nuovo cliente'),
